    ...
    void scan_ports_range(const string& host, int start, int end) {
        ...
        for (int port = start; port <= end; port++) {
            if (check_port(host, port, 1)) {
                ...
                cout << "[LUX PROBE] Port " << port << " open - " << banner << endl;
            }
        }
        active_threads--;
    }
...
public:
    ...
    void scan(int start_port = 1, int end_port = 1024, int thread_count = 50) {
        if (!is_host_up(target)) {
            cout << "[LUX PROBE] Host " << target << " is down" << endl;
            return;
        }
        
        vector<string> ips = resolve_dns(target);
        cout << "[LUX PROBE] DNS resolution for " << target << ":" << endl;
        for (const auto& ip : ips) {
            cout << "  - " << ip << endl;
        }
        
        string main_ip = ips.empty() ? target : ips[0];
        cout << "[LUX PROBE] Scanning ports " << start_port << "-" << end_port << " on " << main_ip << endl;
        ...
        cout << "[LUX PROBE] Scan complete. Found " << open_ports.size() << " open ports" << endl;
    }
    
    vector<int> get_open_ports() { return open_ports; }
};

int main(int argc, char** argv) {
    if (argc < 2) {
        cout << "Usage: " << argv[0] << " <target> [start_port] [end_port]" << endl;
        return 1;
    }
